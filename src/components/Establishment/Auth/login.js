...
import { establishmentLogin } from "../../../services/auth_establishment"

const LoginEstablishment = ({ logged, setLogged }) => {
    ...
    const handleChange = (event) => {
        if (event.target.id === "establishmentId") {
            setError((err) => {
                return { ...err, establishmentId: "" }
            })
    ...
    const handleSubmit = async (event) => {
        ...
        if (handleError()) {
            try {
                ...
                var login = await establishmentLogin(establishmentId, password)
                if (login.success) {
                    setLogged(true)
                } else {
                    setError((err) => {
    ...
    const handleError = () => {
        ...
        if (establishmentId === "") {
            setError((err) => {
                return {
                    ...err,
                    establishmentId: "Establishment ID is required.",
                }
            })
            noError = false
        }
